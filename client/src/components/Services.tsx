...
export default function Services() {
  ...
  useEffect(() => {
    if (expandedService !== null) {
      // Lock the body overflow without using position: fixed which resets scroll
    ...
    return (
      <>
        <motion.div
          ...
          className={`${spanClass || ''} group relative overflow-hidden rounded-2xl bg-gradient-to-br from-white/[0.05] to-white/[0.02] border border-white/10 hover:border-white/20 transition-all duration-500 cursor-pointer`}
        ...
        >
          <div className="relative h-64 overflow-hidden">
            ...
            {/* Expand indicator */}
            <motion.div
              className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
        ...
        <AnimatePresence>
          {isExpanded && (
            ...
            >
                  ...
                  <div className="pt-32 px-8 lg:px-16 pb-20">
                    <div className="grid lg:grid-cols-3 gap-16">
                      ...
                      <div className="lg:col-span-2 space-y-12">
                        ...
                        >
                          <motion.button
                            whileHover={{ scale: 1.02, boxShadow: isDark ? "0 10px 40px rgba(255,255,255,0.1)" : "0 10px 40px rgba(0,0,0,0.2)" }}
                            whileTap={{ scale: 0.98 }}
                            className={`px-8 py-4 rounded-full font-semibold text-lg flex items-center gap-2 transition-all ${
                              isDark 
                          ...
                          {service.details.mainContent.map((paragraph, idx) => (
                            ...
                            >
                              {paragraph.split(/\*\*(.*?)\*\*/g).map((part, i) =>
                                i % 2 === 1 ? (
                                  <strong key={i} className={isDark ? 'text-white' : 'text-black'}>
                                    {part}
                                  </strong>
                                ) : (
                                  part
                        ...
                        <div className="space-y-8">
                          ...
                          <div className={`p-8 rounded-2xl ${
                            ...
                            <motion.button
                              ...
                              whileTap={{ scale: 0.98 }}
                              className={`w-full mt-8 px-6 py-3 rounded-full font-semibold text-sm transition-all ${
                                isDark
                                  ? 'bg-white text-black hover:bg-gray-100'
                                  : 'bg-black text-white hover:bg-gray-900'
                              }`}
              ...
              {/* Back button - Rendered AFTER scrolling container so it sits on top */}
    ...
    >
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          >
            <span className="text-sm text-gray-400 uppercase tracking-wider">OUR CAPABILITIES</span>
          </motion.div>
          
          <h2 className={`text-5xl lg:text-6xl font-bold mb-6 ${isDark ? "text-white" : "text-black"}`}>
            Engineering systems
            <br />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {(() => {
            ...
            const IconComponent = service.icon;
            const isExpanded = expandedService === 0;
            return (
              <>
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                  ...
                  className="lg:col-span-12 group relative overflow-hidden rounded-2xl bg-gradient-to-br from-white/[0.05] to-white/[0.02] border border-white/10 hover:border-white/20 transition-all duration-500 cursor-pointer"
